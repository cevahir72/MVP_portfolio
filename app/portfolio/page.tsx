...
export default function PortfolioPage() {
  ...
  const handleCategoryClick = (categoryId: number) => {
    track("portfolio_view", { action: "category_click", category_id: categoryId });
  ...
  const handleShowcaseClick = (categoryTitle: string, item: ShowcaseItem) => {
    track("portfolio_view", { action: "showcase_click", project_name: item.title, category: categoryTitle });
    if (item.demoUrl) {
  ...
  return (
    <main className="min-h-screen bg-bg font-body text-on-surface">
      {/* Header */}
      <header className="fixed top-0 w-full z-50 bg-bg/80 backdrop-blur-md shadow-sm">
        <div className="flex items-center px-6 py-4 max-w-7xl mx-auto">
          <Link href="/" className="flex items-center gap-2 cursor-pointer group min-h-[44px] py-2">
            <span className="material-symbols-outlined text-secondary group-hover:-translate-x-1 transition-transform">
              arrow_back
            </span>
            <span className="text-xl font-extrabold tracking-tighter text-text font-headline">
              DevArchitect
            </span>
          </Link>
        </div>
      ...
      <div className="pt-28 pb-24 px-6 max-w-7xl mx-auto">
        ...
        <section className="mb-24 flex flex-col items-center">
          <div className="text-center mb-12">
            ...
            <h1 className="text-4xl md:text-5xl font-headline font-bold mt-2 text-text">
              Eşsiz Portföyümüz
            </h1>
            ...
            {portfolios.map((item) => (
              <div key={item.id} className="group h-95 w-full perspective-[1000px] cursor-pointer">
                <div className="relative h-full w-full transform-3d rounded-2xl shadow-xl transition-all duration-700 group-hover:transform-[rotateY(180deg)]">
                  ...
                  {/* Ön Yüz */}
                  <div className="absolute inset-0 flex h-full w-full backface-hidden flex-col overflow-hidden rounded-2xl border border-border bg-bg-card">
                    <div className="w-full h-3/5 bg-bg-card flex items-center justify-center relative overflow-hidden">
                      <Image src={item.image} alt={item.title} fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 20vw" className="object-cover" />
                    </div>
                    <div className="p-4 flex-1 flex flex-col justify-center items-center text-center">
                      <h3 className="text-xl font-bold font-headline text-text leading-tight">{item.title}</h3>
                  ...
                  <div className="absolute inset-0 flex h-full w-full transform-[rotateY(180deg)] flex-col items-center justify-center rounded-2xl bg-linear-to-br from-secondary to-blue-400 p-6 text-center text-white backface-hidden">
                    ...
                    {item.demoUrl ? (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="cursor-pointer px-5 py-3 min-h-[48px] bg-white text-secondary rounded-full font-bold text-xs hover:scale-105 active:scale-95 transition-transform shadow-md flex items-center justify-center"
                      >
                        {item.btnText}
                      </a>
                    ) : (
        ...
        <section className="pt-16 border-t border-border">
          <div className="text-center mb-16 max-w-3xl mx-auto">
            <h2 className="text-3xl md:text-4xl font-headline font-bold text-text">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 md:auto-rows-[200px] gap-4 md:gap-6">
            {categories.map((category) => (
              <div 
                ...
                className={`group relative bg-bg-card rounded-3xl p-6 shadow-sm border cursor-pointer overflow-hidden transition-all duration-300 hover:shadow-2xl hover:-translate-y-1 flex flex-col justify-end ${category.colSpan} ${selectedCategoryId === category.id ? "border-secondary shadow-xl ring-2 ring-secondary/20" : "border-border hover:border-primary/40"}`}
              >
                {/* Arka Plan Fotoğraf Alanı */}
                <div className="absolute inset-0 bg-bg-card flex items-center justify-center overflow-hidden">
                  <span className="material-symbols-outlined text-5xl text-text-muted group-hover:scale-110 group-hover:text-primary transition-all duration-300 z-10">
                    add_photo_alternate
                  </span>
                  {/* FOTOĞRAF EKLENECEK ALAN */}
                  <Image src={category.img} alt={category.title} fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" className="object-cover z-0 group-hover:scale-105 transition-transform duration-500" />
                  
                  {/* Gradient Overlay for Text Readability */}
                  <div className="absolute inset-0 z-10 bg-linear-to-t from-bg/90 via-bg/30 to-transparent"></div>
                </div>
          ...
          <div id="category-showcase" className="mt-12 rounded-[32px] border border-border bg-bg-card p-6 md:p-10 shadow-[0_24px_80px_var(--shadow-card)] scroll-mt-32">
            <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
              <div className="max-w-3xl">
                <span className="text-secondary font-bold tracking-widest text-xs uppercase font-label">
                  Seçili Kategori
                </span>
                <h3 className="mt-2 text-3xl md:text-4xl font-headline font-bold text-text">
              ...
              <div className="inline-flex items-center gap-3 self-start rounded-full bg-bg-card px-4 py-3 text-sm font-semibold text-text">
            ...
            <div className="mt-10 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {activeCategory.examples.map((item) => (
                <article
                  key={item.id}
                  className="group overflow-hidden rounded-[28px] border border-border bg-bg-card shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl"
                >
                  <div className="relative h-56 overflow-hidden bg-bg-card">
                    <Image
                      src={item.image}
                      alt={item.title}
                      fill
                  ...
                  <div className="flex h-[260px] flex-col p-6">
                    <h4 className="text-xl font-headline font-bold text-text">
                      {item.title}
                    </h4>
                    ...
                    <div className="mt-auto flex items-center justify-between gap-4 pt-6">
                      ...
                      <button
                        ...
                        className={`cursor-pointer rounded-full px-5 py-3 min-h-[48px] text-xs font-bold transition-transform active:scale-95 flex items-center justify-center ${item.demoUrl ? "bg-secondary text-white hover:scale-105" : "bg-bg text-white hover:scale-105"}`}
                      >
